...
**Version 2.2.13** restores the original native ScriptUI Import Assets callback and keeps a typed-value fallback, so pasted Sourcing paths reach the importer reliably. It accepts normal file paths, file URLs, quoted paths, cross-platform root mapping, and folder-plus-filename lists without restricting imports to a short file-extension list. Missing or invalid entries are reported while the remaining valid files import.
...
1. Download `AE-Toolkit-v2.2.13.zip` from the [latest release](https://github.com/danrac/AE-Toolkit/releases/latest). You can also download or clone this repository.
