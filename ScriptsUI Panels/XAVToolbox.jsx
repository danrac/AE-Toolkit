(function XAVToolbox(thisObj) {
    ...
    #include "XAVToolbox_Assets/HelperScripts/UTILITY_Functions.jsx";
    #include "XAVToolbox_Assets/HelperScripts/UTILITY_DateHandler.jsx";
   ...
   /////////// COVER CHECKERS //////////
    ...
    function XAVToolbox_buildUI(thisObj) {
        ...
        if (pal != null) {
            ...
            createLog("CreateModify/Default_StudioList", StudioCMDDList, "Library");
            createLog("CreateModify/Default_SizeList", SizeCMDDList, "Library");

            StudioCMDDList = readLog('CreateModify/Default_StudioList', 'Library').split('\n');           
            SizeCMDDList = readLog('CreateModify/Default_SizeList', 'Library').split('\n');

            var mainCMPanel = createmodgroup.add('Group', undefined, '');
            mainCMPanel.orientation = 'column';
            mainCMPanel.alignChildren = ['left', 'top'];
            ...
            var studioCMLable = StudioCMGroup.add("StaticText", undefined, "CLIENT SPECS:");
            ...
            StudioCMDD.onChange = function(){
                if(StudioCMDD.selection != 0){
                    ...
                    createLog('CreateModify/' + currentStudioNameCM + '_SizeList', SizeCMDDList, "Library");    
                    SizeCMDDList = readLog('CreateModify/' + currentStudioNameCM + '_SizeList', 'Library').split('\n');
                    SizeCMDD.removeAll();
                    for (var i = 0; i <= SizeCMDDList.length - 1; i++) {
                        var tmpArr = SizeCMDDList[i].split(',');
                        SizeCMDD.add("item", tmpArr[0]);
...
////CREATE / MODIFY///////////
    ...
    function CreateCompSetup(inputArray){
        CreateComp(1, inputArray[1], inputArray[2], inputArray[3], parseInt(inputArray[4]), parseInt(inputArray[5]), parseInt(inputArray[6]), parseInt(inputArray[7]), inputArray[8], inputArray[9], inputArray[16], inputArray[18]);
    }

    function CreateComp(numberid, newchart, newhdchart, newmatte, sizeX, sizeY, offsetX, offsetY, linereturn, textalignment, nameprefix, tmpARname){
        appendLog("Function_Tracking", currentDateYMD + " " + userName + " // :: Function Name: createXAVComp // :: //", "Log");
            ...
            var setTextJust = textJustification(textalignment);
            ...
            var newXAVComp = app.project.items.addComp(job + "_" + compARname + "_" + style + "_" + desc + "_" + init + "_01", sizeX, sizeY, 1, 10, framerateSelection);

            if(guidescbenabled == true && customInputs == false){
                var chartLayerImport = ChartCheck(newchart);
                var chartLayer = newXAVComp.layers.add(chartLayerImport);
                ...
                if(newmatte != null){
                    var matteLayer = ChartCheck(newmatte);
                    var tmpMatteLayer = newXAVComp.layers.add(matteLayer);
                    tmpMatteLayer.transform.position.expression = "[(thisComp.width / 2),(thisComp.height / 2)];";
                    tmpMatteLayer.guideLayer = true;
                }
                if(newhdchart != null){
                    var chartHDLayer = ChartCheck(newhdchart);
                    var HDchartLayer = newXAVComp.layers.add(chartHDLayer);
                ...
                textDocument1.text = "";
                textDocument1.justification = setTextJust;
    ...
    function ModifyCompSetup(inputArray){
            ...
            for (var i = 0; i <= selectedComps.length - 1; i++){                    
                ...
                ModifyComp(i, inputArray[1], inputArray[2], inputArray[3], parseInt(inputArray[4]), parseInt(inputArray[5]), parseInt(inputArray[6]), parseInt(inputArray[7]), inputArray[8], inputArray[9], inputArray[16], inputArray[18]);
            }

    }

    function ModifyComp(numberid, newchart, newhdchart, newmatte, sizeX, sizeY, offsetX, offsetY, linereturn, textalignment, nameprefix, tmpARname){
        ...
        if(selectedComps.length > 0){
            for (var x = 0; x < selectedComps.length; x++){
                ...
                for(var y = 2; y <= layerCollection.length; y++){
                    if(layerCollection[y].name != newmatte && layerCollection[y].name != newchart && layerCollection[y].name != "XAV frame counter info" && layerCollection[y].name != "XAVoffset"){
                ...
                if(sizecbenabled == true){
                    ...
                    if(guidescbenabled == true && customInputs == false){
                        ...
                        currlayer.transform.position.setValue([offsetX, offsetY]); 

                        if(newmatte != ""){   
                            if(selectedComps[x].layer(tmpMatteLayer)){
                                tmpMatteLayer = selectedComps[x].layer(tmpMatteLayer);
                                tmpMatteLayer.transform.position.expression = "[(thisComp.width / 2),(thisComp.height / 2)];";
                                tmpMatteLayer.guideLayer = true; 
                            }else{
                                matteLayer = ChartCheck(newmatte);
                                tmpMatteLayer = selectedComps[x].layers.add(matteLayer);
                                tmpMatteLayer.transform.position.expression = "[(thisComp.width / 2),(thisComp.height / 2)];";
                                tmpMatteLayer.guideLayer = true; 
                            }
                            if(selectedComps[x].layer(tmpChartLayer)){
                                tmpChartLayer = selectedComps[x].layer(tmpChartLayer);
                                tmpChartLayer.transform.position.expression = "[(thisComp.width / 2),(thisComp.height / 2)];";
                            }else{
                                chartLayer = ChartCheck(newchart);
                                tmpChartLayer = selectedComps[x].layers.add(chartLayer);
                                tmpChartLayer.transform.position.expression = "[(thisComp.width / 2),(thisComp.height / 2)];";
                            }

                            if(newhdchart != ""){
                                if(selectedComps[x].layer(chartHDLayer)){
                                    tmpChartHDLayer = selectedComps[x].layer(tmpChartHDLayer);
                                    tmpChartHDLayer.remove();
                                    // tmpChartHDLayer.transform.position.expression = "[(thisComp.width / 2),(thisComp.height / 2)];";
                                }else{
                                    chartHDLayer = ChartCheck(newhdchart);
                                    tmpChartHDLayer = selectedComps[x].layers.add(chartHDLayer);
                                    tmpChartHDLayer.transform.position.expression = "[(thisComp.width / 2),(thisComp.height / 2)];";
                                }
                                tmpChartHDLayer.property("Opacity").setValue(50);
                                tmpChartHDLayer.guideLayer = true;
                            }
                        }
                        else{
                            if(selectedComps[x].layer(tmpChartLayer)){
                            ...
                            if(newhdchart != ""){
                                if(selectedComps[x].layer(tmpChartHDLayer)){
                                    tmpChartHDLayer = selectedComps[x].layer(tmpChartHDLayer);
                                    tmpChartHDLayer.remove();
                                    tmpChartHDLayer.transform.position.expression = "[(thisComp.width / 2),(thisComp.height / 2)];";
                                }else{
                        ...
                        tmpChartLayer.property("Opacity").setValue(50);
                        tmpChartLayer.guideLayer = true;
...
////CHECKERS AND COVERS///////
    ...
    function BuildChecker(numberid, newchart, newhdchart, newmatte, sizeX, sizeY, offsetX, offsetY, linereturn, textalignment, nameprefix, ltx, lty, lta, stx, sty, sta){
        ...
        var currentMatte;
        if(newchart != ""){
            currentChart = ChartCheck(newchart);
        }
        if(newhdchart != ""){
            currentHDChart = ChartCheck(newhdchart);
        }     
        if(newmatte != ""){
            currentMatte = ChartCheck(newmatte);
        }        
