...
2.2.15 repairs the remaining Sourcing > Import Assets field lookup issue. The import button reads the paste field directly, so it no longer depends on ScriptUI button event data. The field is blank by default, uses a tooltip for guidance, and retains typed input as a fallback.
