...
(function Captioneer(thisObj)
{
	...
	var CaptioneerData = new Object();	// Store globals in an object
	CaptioneerData.scriptName = "Captioneer";
	CaptioneerData.scriptTitle = CaptioneerData.scriptName + " v1.0";
	CaptioneerData.maxTextChars = 255;		// Maximum number of characters supported by edittext control
	CaptioneerData.strTextHeading = {en: "Text (up to "+CaptioneerData.maxTextChars+" characters):"};
	CaptioneerData.strHelp = {en: "?"}
	CaptioneerData.strErrNoCompSel = {en: "Cannot perform operation. Please select or open a single composition in the Project window, and try again."};
	CaptioneerData.strErrNoSingleTextLayerSel = {en: "Cannot perform operation. Please select a single text layer, and try again."};
	CaptioneerData.strErrNoTextLayersSel = {en: "Cannot perform operation. Please select at least one text layer, and try again."};
	CaptioneerData.strMinAE100 = {en: "This script requires Adobe After Effects CS5 or later."};
        
    	function Captioneer_localize(strVar)
    	{
    		return strVar["en"];
    	}
    	
    	function Captioneer_buildUI(thisObj)
    	{
            var pal = (thisObj instanceof Panel) ? thisObj : new Window("palette", CaptioneerData.scriptName, undefined, {resizeable:true});		
    	...
    	function generateMarkers(){
    		    app.beginUndoGroup(CaptioneerData.scriptName);
        ...
        if (parseFloat(app.version) < 10.0)
            alert(Captioneer_localize(CaptioneerData.strMinAE100), CaptioneerData.scriptName);
        else{
            var rdetPal = Captioneer_buildUI(thisObj);
            if (rdetPal !== null){
